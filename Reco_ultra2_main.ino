...
bool acq_ultra_pin() {
  ...
  if (waitingForPulse[j] == false) { // envoyer impulsion
    ...
    // Vérifier si le timeout (5ms) est dépassé
      ...
      } else  if (pulseStart_bool[j] && digitalRead(echo) == LOW) {
        unsigned long duration = currentTime - pulseStart_val[j]; // Calculer la durée
        distance = duration / 29.1 / 2; // Convertir en distance (cm)
...
const uint8_t en_engine_matrix[5][4][2] = {
  ...
  {{0, 255 }, {255, 0 }, {0 , 0}, {0 , 0}},                         // ultra      [0-1: {0: pos; 1: pos}]                        (2-3 unused)

  {{255, 255 }, {255, 255 }, {0 , 0}, {0 , 0}},                     // ultra mod　　［０－1： ｛０： pos； 1： pos｝］　　　　　　　　　　　　　
  {{255, 255}, {255, 255}, {255, 255}, {255, 255}}                  // ctr        [0-1: {0: neg; 1: neg}; 2-3: {0: pos; 1: pos}]
};

const uint8_t engine_en_pin[2] =  {6, 3};  // [0: left; 1: right]
const uint8_t engine_dir_pin[2] = {9, 5}; // [0: left; 1: right]
...
const uint8_t dir_engine_matrix[5][4][2] = {
  {{255, 255},  {255, 255}, {0, 0}, {0, 0}},                                  // ctr        [0-255: {0: neg; 1: neg}; 2-3: {0: pos; 1: pos}]
  {{ 255, 0}, {0, 255}, {0, 0}, {0, 0}},                                  // laser      [0  : {0: neg; 1: pos}; 1: {0: pos; 1: neg}]   (2-3 unused)
  {{ 0, 0}, {0, 0}, {0, 0}, {0, 0}},                                  // ultra      [0-1: {0: pos; 1: pos}]                        (2-3 unused)

  {{0, 0}, {0, 0}, {0, 0}, {0, 0}},                                   // ultra mod  [0-1: {0: pos; 1: pos}]                        (2-3 unused)
  {{255, 255}, {255, 255}, {0, 0}, {0, 0}}                                    // ctr        [0-1: {0: neg; 1: neg}; 2-3: {0: pos; 1: pos}]
};
...
void loop() {
  in = true;
  for (i = 2; i < 3 && in; i++) {  // i increments before break
